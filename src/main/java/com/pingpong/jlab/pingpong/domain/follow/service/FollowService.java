...
public class FollowService {
    ...
    public ApiResponse getUserFollowerList(PaginationRequestDto dto , String userId){
        Optional<User> user = userRepository.findByUserid(userId);
        if(user.isEmpty()){
            return ApiResponse.res(400, ErrorCode.USER_NOT_FOUND.getMessage());
        }
        return ApiResponse.res(200,"유저 팔로워 리스트",followRepository.getFollowerListByUser(user.get(),dto));
    }
